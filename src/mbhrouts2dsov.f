...
c     lie on a circle
     ...
     2           ifder5,der5)
            if (ifcharge .eq. 1) then
               ...
               up(i) = up(i) + dipstr(j)*(zcirc(1,i)*grad(1) 
     1              + zcirc(2,i)*grad(2))
            endif
     ...
     6              zcirc(2,i)*der4(5)))
      ...
      subroutine mbh2dsov_dtocmp(beta,rad,rscale,u,up,npts,coeffs1,
     1     coeffs2,wsave,work)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
...
c     OUTPUT
c
c     coeffs1, coeffs2 - REAL *8 arrays of length npts which 
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc    
      implicit none
      real *8 beta, rad, u(*), up(*), coeffs1(*), coeffs2(*), work(*)
      real *8 wsave(*), rscale
      integer npts
c     local variables
     ...
     1     coeffs1,coeffs2,wsave,work(idiffs),work(iders),
     ...
     1     coeffs1,coeffs2,wsave,diffs,ders,hvec,hder,
     2     kvec,kder,cu,cup)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real *8 beta, rad, u(*), up(*), coeffs1(*), coeffs2(*)
...
c     for radius of circle
      ...
      call mbh2dsov_rk(pow,dpow,rad,beta,rscale,l)
...
c     sin(j theta) parts by DFFTF
      ...
      subroutine mbh2dsov_dtocmp_naive(beta,rad,rscale,u,up,npts,
     1     coeffs1,coeffs2,wsave,work)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c     This is a wrapper routine. The real work is carried out by 
c     MBH2DSOV_DTOC1. 
...
c     local variables
      ...
      call mbh2dsov_dtocmp_naive1(beta,rad,rscale,u,up,npts,
     1     coeffs1,coeffs2,wsave,work(idiffs),work(iders),
     2     work(ihvec),work(ihder),work(ikvec),work(ikder),
     3     work(icu),work(icup))
      ...
      subroutine mbh2dsov_dtocmp_naive1(beta,rad,rscale,u,up,npts,
     1     coeffs1,coeffs2,wsave,diffs,ders,hvec,hder,
     2     kvec,kder,cu,cup)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
...
c     local variables
      ...
      subroutine mbh2dsov_dtocta_naive(beta,rad,rscale,u,up,npts,
     1     coeffs1,coeffs2,wsave,work)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c     This is a wrapper routine. The real work is carried out by 
...
c     local variables
      ...
      call mbh2dsov_dtocta_naive1(beta,rad,rscale,u,up,npts,
     1     coeffs1,coeffs2,wsave,work(idiffs),work(iders),
     2     lwfjs,work(ifjs),work(ifjder),work(iiscale),
     3     work(iivec),work(iider),
     4     work(icu),work(icup))
      ...
      subroutine mbh2dsov_dtocta_naive1(beta,rad,rscale,u,up,npts,
     1     coeffs1,coeffs2,wsave,diffs,ders,lwfjs,fjs,fjder,
     2     iscale,pow,dpow,cu,cup)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c     see MBH2DSOV_DTOCTA for description
...
c     copy solution to correct entries
      ...
      subroutine mbh2dsov_rk(pow,dpow,r,beta,rscale,nterms)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c     INPUT:
c
...
c     local
      ...
      subroutine mbh2dsov_convert_coeffs(coeffs1,coeffs2,nterms)

      implicit none
c     global
      integer nterms
      real *8 coeffs1(*), coeffs2(*)
c     local
      integer i
